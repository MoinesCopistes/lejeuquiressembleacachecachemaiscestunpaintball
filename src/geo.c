...
int p_point_is_in_circle(Position *pos, Circle *circle) {
  if (p_distance_position_square(pos, &(circle->pos)) <=
...
int p_circle_is_in_circle(Circle *circle1, Circle *circle2) {
  ...
  if (p_distance_position_square(&(circle1->pos), &(circle2->pos)) <
...
int p_point_is_in_rect(Position *pos, Rect *rect) {
  if ((pos->x > rect->pos.x) && (pos->x < rect->pos.x + rect->w) &&
      (pos->y > rect->pos.y) && (pos->y < rect->pos.y + rect->h))
    return 1;
  return 0;
}
...
Position cursor_with_offset(Vector2 vec) {
  Position pos;
  pos.x = (float)vec.x + (int)world.offset.x;
  pos.y = (float)vec.y + (int)world.offset.y;
  return pos;
}
